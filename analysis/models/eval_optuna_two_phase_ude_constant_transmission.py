...
if base_config["n_days_pred_conc"]>0:
    model_path = pathlib.Path(f"Bonn_constant_transmission/optuna_best_{base_config['phase_cut_date']}_{args.objective}_pred_{base_config['n_days_pred_conc']}d_conc")
else:
    model_path = pathlib.Path(f"Bonn_constant_transmission/optuna_best_{base_config['phase_cut_date']}_{args.objective}")
...
model = two_phase_integrative_ude.IntegrativeModel(
    ...
    initial_transmission=config.get("initial_transmission", 0.0),
    ) # days, delay between infection and reporting
...
meta = {
            ...
            "transmission_rate": float(model.UDE.rhs.transmission_rate),
        }
...
def get_hp_config_from_trial(study_df_row, base_config):
    ...
    hp_config = {
        ...
        "initial_transmission": float(study_df_row["params_initial_transmission"]), # initial constant transmission rate
    }
...
storage = f"sqlite:///Bonn_constant_transmission/optuna_study_two_phase_model_{config['phase_cut_date']}.db"
...
for i in range(1, 10):
    ...
    model, total_negll, train_negll, val_negll, train_loss = optimization_utils.two_phase_integrative_model_constant_transmission_train_and_evaluate(new_config, None, "todo", print_every=100, trial=new_config["trial_number"])
    train_negll_I, train_negll_c, train_negll = per_observable_likelihood(model, data["t_all"], data["t_phase_1"], data["t_mask_I_train"], data["t_mask_conc_train"], data["I_train"], data["conc_train"])
    val_negll_I, val_negll_c, val_negll = per_observable_likelihood(model, data["t_all"], data["t_phase_1"], data["t_mask_I_val"], data["t_mask_conc_val"], data["I_val"], data["conc_val"])
    total_negll_I, total_negll_c, total_negll = per_observable_likelihood(model, data["t_all"], data["t_phase_1"], data["t_mask_I_all"], data["t_mask_conc_all"], data["I_all"], data["eval_conc"])
    test_negll_I, _, _ = per_observable_likelihood(model, data["t_all"], data["t_phase_1"], data["t_mask_I_test"], data["t_mask_conc_all"], data["I_test"], data["eval_conc"])
