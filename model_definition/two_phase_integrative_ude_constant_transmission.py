...
class UDE_rhs(eqx.Module):
    ...
    constant_transmission: bool = eqx.field(static=True)


    def __init__(self, state_dim, width_size, depth, activation, 
                 n_freqs, t_scale, population_size, *, key, initial_transmission=0.3, constant_transmission=True, **kwargs):
        ...
        self.transmission_rate = initial_transmission
        self.constant_transmission = constant_transmission
    ...
    def __call__(self, t, y, args):
        ...
        S, E, I, R, cum_I_new = y
        if self.constant_transmission:
            beta = self.transmission_rate*self.t_scale
        else:
...
class NeuralUDE(eqx.Module):
    ...
    def __init__(self, state_dim, width_size, depth, activation, n_freqs, t_scale, population_size, E0_init, I0_init, R0_init,
                 solver, solver_kwargs, initial_transmission, *, key, **kwargs):
        super().__init__(**kwargs)
        self.rhs = UDE_rhs(state_dim, width_size, depth, activation, n_freqs, t_scale, population_size, initial_transmission=initial_transmission, constant_transmission=True, key=key)
        self.solver = solver
...
class IntegrativeModel(eqx.Module):
    ...
    def __init__(self, width_size, depth, activation, n_freqs, t_scale, population_size, E0_init, I0_init, R0_init,
                 solver, solver_kwargs, k1_init, k2_init, k3_init, T_peak_init, T_max, initial_transmission, *, key,  init_par_vmr: float = 0.1,
        ...
        self.UDE = NeuralUDE(1, width_size, depth, activation, n_freqs, t_scale, population_size,
                             ...
                             solver=solver, solver_kwargs=solver_kwargs, initial_transmission=initial_transmission, key=beta_key)
...
def plot_beta_prediction(model, t_all, dates_all, phase_cut_date):    
    ...
    fig, ax = plt.subplots(ncols=1, figsize=(4.5, 2.5), dpi=300)
    ax.plot(dates_all, jnp.repeat(jnp.array([model.UDE.rhs.transmission_rate]), len(dates_all)), c="saddlebrown")
...
def plot_effective_reproduction_number(model, t_all, dates_all, phase_cut_date):
    pred = model.UDE(t_all)[:,:,0]
    beta_pred = jnp.repeat(jnp.array([model.UDE.rhs.transmission_rate]), len(dates_all))
