...
class UDE_rhs(eqx.Module):
    ...
    def __call__(self, t, y, args):
        ...
        if len(self.freqs)>0:
            t_feat = self.fourier_features(t/self.t_scale) 
        else:
            t_feat = jnp.atleast_1d(t/self.t_scale)
...
class IntegrativeModel(eqx.Module):
    ...
    def _beta_scalar(self, t):
        ...
        rhs = self.UDE.rhs
        if len(rhs.freqs) > 0:
            x = rhs.fourier_features(t/rhs.t_scale)                 # shape (2*n_freqs + 1,)
        else:
            x = jnp.array([t/rhs.t_scale])                          # shape (1,)
