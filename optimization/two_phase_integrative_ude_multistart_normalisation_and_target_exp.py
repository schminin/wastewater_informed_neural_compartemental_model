...
if base_config["n_days_pred_conc"]>0:
    optuna_path = pathlib.Path(f"Bonn_{args.substance_normalization}_{args.gene_target}/optuna_best_{base_config['phase_cut_date']}_{args.objective}_pred_{base_config['n_days_pred_conc']}d_conc")
else:
    optuna_path = pathlib.Path(f"Bonn_{args.substance_normalization}_{args.gene_target}/optuna_best_{base_config['phase_cut_date']}_{args.objective}")
...
def per_observable_likelihood(model, t_all, t_phase_1, t_mask_ids_I, t_mask_ids_conc, obs_cases, obs_conc):
    ...
    return (nll_I / I_mask), (nll_C / C_mask), (nll_I / I_mask) + (nll_C / C_mask), I_mask, C_mask, 
...
def objective(seed):
    ...
    if base_config["n_days_pred_conc"]>0:
        outdir = pathlib.Path(f"Bonn_{args.substance_normalization}_{args.gene_target}/multistart_models/{args.phase_cut_date}_{args.objective}_pred_{base_config['n_days_pred_conc']}d_conc")
    else:
        outdir = pathlib.Path(f"Bonn_{args.substance_normalization}_{args.gene_target}/multistart_models/{args.phase_cut_date}_{args.objective}")
