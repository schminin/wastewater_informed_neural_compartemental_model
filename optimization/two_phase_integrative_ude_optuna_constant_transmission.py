...
config["n_days_pred_conc"] = args.n_days_pred_conc  # days to predict beyond last observed date for concentration
...
def objective(trial):
    hp_config = {
        ...
        "initial_transmission": trial.suggest_float("initial_transmission", -0.3, 0.3),  # initial constant transmission rate
    }
    ...
    model, total_nll, train_nll, val_nll, train_loss = optimization_utils.two_phase_integrative_model_constant_transmission_train_and_evaluate(config, None, "todo", print_every=100, trial=trial.number)    
    ...
    if is_new_best:
        ...
        if config["n_days_pred_conc"]>0:
            outdir = pathlib.Path(f"Bonn_constant_transmission/optuna_best_{config['phase_cut_date']}_{args.objective}_pred_{config['n_days_pred_conc']}d_conc")
        else:
            outdir = pathlib.Path(f"Bonn_constant_transmission/optuna_best_{config['phase_cut_date']}_{args.objective}")
...
pathlib.Path("Bonn_constant_transmission").mkdir(parents=True, exist_ok=True)
if config["n_days_pred_conc"]>0:
    storage = f"sqlite:///Bonn_constant_transmission/optuna_study_two_phase_model_{config['phase_cut_date']}_pred_{config['n_days_pred_conc']}d_conc.db?timeout=600&journal_mode=WAL"
else:
    storage = f"sqlite:///Bonn_constant_transmission/optuna_study_two_phase_model_{config['phase_cut_date']}.db?timeout=600&journal_mode=WAL"
